...
namespace MinDbg.CorDebug
{
    ...
    public sealed class CorProcess : CorController
    {
        ...
        /// Handler for CorBreakpoint event.
        /// </summary>
        public delegate void CorExceptionHandler(CorExceptionEventArgs ev);

        /// <summary>
        /// Occurs when breakpoint is hit.
        /// </summary>
        public event CorExceptionHandler OnException;
                ...
                if (symreader != null)
                {
                    foreach (ISymbolDocument symdoc in symreader.GetDocuments())
                    {
                        if (String.Compare(symdoc.URL, fileName, true, CultureInfo.InvariantCulture) == 0 ||
                            String.Compare(System.IO.Path.GetFileName(symdoc.URL), System.IO.Path.GetFileName(fileName), true, CultureInfo.InvariantCulture) == 0)
                        {
                            Int32 line = 0;
                            try
                            {
